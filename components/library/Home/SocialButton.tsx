...
const SocialButton = ({ icon, label, isSelected, onClick }: SocialButtonProps) => {
  return (
    <button
      ...
      `}
    >
      <Image src={icon} alt={label} width={4} height={4} className={`w-4 h-4 transition-all duration-200 ${isSelected ? "brightness-0 invert" : "opacity-70 group-hover:opacity-100"}`} />
