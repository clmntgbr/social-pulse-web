...
export async function getFacebookLoginUrl(token: string, dispatch: Dispatch<SocialAccountsActionTypes>): Promise<GetLoginUrl | null> {
  try {
    ...
    if (response === null) {
      dispatch({
        type: SocialAccountsAction.GET_FACEBOOK_LOGIN_URL_HTTP_INTERNAL_ERROR,
    ...
    switch (response.status) {
      ...
      case HTTP_STATUS.NOT_FOUND:
        dispatch({
          type: SocialAccountsAction.GET_FACEBOOK_LOGIN_URL_NOT_FOUND,
    ...
    dispatch({
      type: SocialAccountsAction.GET_FACEBOOK_LOGIN_URL_ERROR,
