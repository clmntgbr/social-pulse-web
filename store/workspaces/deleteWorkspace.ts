...
export async function deleteWorkspace(token: string, workspaceUuid: string, dispatch: Dispatch<WorkspacesActionTypes>): Promise<PromiseResponse> {
  try {
    dispatch({ type: WorkspacesAction.LOADING_START });

    const client = new SocialPulseClient(token);
    const response = await client.deleteWorkspace(workspaceUuid);

    if (response === null) {
      dispatch({
        type: WorkspacesAction.DELETE_WORKSPACE_HTTP_INTERNAL_ERROR,
    ...
    switch (response.status) {
      ...
      case HTTP_STATUS.NOT_FOUND:
        dispatch({
          type: WorkspacesAction.DELETE_WORKSPACE_NOT_FOUND,
    ...
    dispatch({
      type: WorkspacesAction.DELETE_WORKSPACE_ERROR,
