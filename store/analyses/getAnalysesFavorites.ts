...
export async function getAnalysesFavorites(token: string, dispatch: Dispatch<AnalysisActionTypes>): Promise<void> {
  try {
    dispatch({ type: AnalysisAction.ANALYSIS_LOADING_START });

    const client = new ApiClient(token);
    const response = await client.getAnalysesFavorites();

    if (response === null) {
      dispatch({
        type: AnalysisAction.GET_ANALYSES_FAVORITES_ERROR,
    ...
    switch (response.status) {
      ...
      case HttpStatus.NOT_FOUND:
        dispatch({
          type: AnalysisAction.GET_ANALYSES_FAVORITES_NOT_FOUND,
      ...
      default:
        dispatch({
          type: AnalysisAction.GET_ANALYSES_FAVORITES_HTTP_INTERNAL_ERROR,
